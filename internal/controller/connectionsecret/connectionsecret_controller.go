...
func (r *ConnectionSecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	var connSecret ConnectionSecretContext

	// Retrive ProjectID, ClusterName, DatabaseUsername. Also ProjectName if this request was not sent by watchers
	if err := connSecret.ParseRequestName(ctx, r.Client, req.NamespacedName); err != nil {
	...
	// Retrive the AtlasDeployment and AtlasDatabaseUser using indexers
	if err := connSecret.LoadDeploymentAndUser(ctx, r.Client, req.Namespace); err != nil {
	...
	// Check that both resources AtlasDeployment and AtlasDatabaseUser are ready
	if ready, notReady := connSecret.AreResourcesReady(); !ready {
		return workflow.InProgress("ConnectionSecretNotReady", fmt.Sprintf("Not ready: %s", strings.Join(notReady, ", "))).ReconcileResult()
	}

	// Check if we have projectName. This is important as all ConnectionSecrets
	// require the ProjectName for constructing their namespace/name.
	if !connSecret.HasProjectName() {
		if connSecret.ShouldUseSDKResolution() {
			connectionConfig, err := r.ResolveConnectionConfig(ctx, connSecret.Deployment)
			if err != nil {
			...
			if err := connSecret.ResolveProjectNameSDK(ctx, projectService); err != nil {
				r.Log.Errorw("Failed to fetch project from Atlas API", "projectID", connSecret.ProjectID, "error", err)
				return workflow.Terminate("FailedToFetchProjectFromAtlas", err).ReconcileResult()
			}
		} else {
			if err := connSecret.ResolveProjectNameK8s(ctx, r.Client, req.Namespace); err != nil {
				r.Log.Errorw("Failed to retrieve AtlasProject to resolve ProjectName", "projectID", connSecret.ProjectID, "error", err)
				return workflow.Terminate("FailedToResolveProjectName", err).ReconcileResult()
			}
		}
	}

	data, err := connSecret.BuildConnectionData(ctx, r.Client)
	if err != nil {
	...
	if err := connSecret.handleSecret(ctx, r.Client, data); err != nil {
	...
	r.Log.Infow("Successfully found associated resources",
		"deployment", connSecret.Deployment.GetDeploymentName(),
		"user", connSecret.User.Spec.Username,
		"projectID", connSecret.ProjectID,
	)
